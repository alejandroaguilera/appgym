...
export interface SeedResult {
  ...
  bodyMetricsCreated: number;
}

// Idempotent: no-ops if a Block already exists. Seeds Block 1 + Sessions
// A-D exactly as specified in APP-GYM-SPEC.md §11 (the real training plan),
// plus synthetic BodyMetric rows standing in for the real Omron CSV
// referenced there, which doesn't exist in this environment.
export async function seedDatabase(prisma: PrismaClient): Promise<SeedResult> {
  const existingBlocks = await prisma.block.count();
  if (existingBlocks > 0) {
    return { skipped: true, exercisesCreated: 0, sessionsCreated: 0, bodyMetricsCreated: 0 };
  }
  ...
  return {
    skipped: false,
    exercisesCreated: exerciseByName.size,
    sessionsCreated: sessions.length,
    bodyMetricsCreated: bodyMetrics.length,
  };
}
